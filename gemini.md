...
- [x] 深度优化 `SectorBiddingPanel` UI 响应式架构：
  ...
  - [x] **修复 UI 持久化与代码损坏**：针对重构过程中出现的代码冲突和损坏，进行了手术级修复。完整恢复了 `_save_ui_state` 和 `_restore_ui_state` 方法，确保手动调整的列宽和分割线位置在重启后依然生效。
...
## 2026-04-08 11:50
- [x] 深度优化表格排序与滚动回顶交互：
  - [x] **强制手动排序回顶**：修改了板块表、个股表、重点表的表头点击回调，移除之前仅在焦点切换时回顶的动态逻辑。现在任何手动点击表头排序的操作都将触发 `reset_to_top=True`，确保立即展示最强/最弱的极值个股。
...
## 2026-04-08 21:15
- [x] 深度修复 `bidding_momentum_detector.py` 持久化与复盘逻辑：
...
- [x] 深度优化 `SectorBiddingPanel` 搜索逻辑，转向**板块溯源模式**：
    ...
    - [x] **深度联动与过滤解除**：优化了重点表的点击行为。用户点击溯源出的板块记录时，系统会自动在左侧定位跳选该板块。同时，**临时解除个股视图的搜索词过滤限制**，确保上方个股明细表能完整展示该板块的所有跟随股（而非仅显示搜索的个股），极大提升了复盘效率。
...
- [x] 深度修复 `BiddingMomentumDetector` 跨日数据残留逻辑：
    - [x] **实现多维触发时间判定 (Multi-source Trigger Logic)**：在 `daily_watchlist` 中补齐了 `trigger_ts` 持久化字段，并将 `_prune_expired_signals` 侦测范围扩展至重点表与活跃板块全量时间戳。
    ...
    - [x] **增强自愈清理深度 (Deep Self-healing)**：清理逻辑现在包含 `_sector_active_stocks_persistent` 增量缓存，杜绝了“僵尸板块”在清空 `active_sectors` 后由于增量刷新而死灰复燃的可能。

## 2026-04-09 12:20
- [x] 深度修复 `BiddingMomentumDetector` 当日重点表跨日数据残留：
    - [x] **实现记录级时间戳验证 (Entry-level Timestamp Validation)**：在加载过程中对 `daily_watchlist` 每一项进行 `trigger_ts` 校验，强制剔除早于今日零点的记录，彻底解决了“启动后文件被今日时间戳污染导致加载昨日旧数据”的顽疾。
    - [x] **增强日期字符串识别**：支持对 `time_str` (如 "0408-15:04") 进行子串检测，自动识别并丢弃包含昨日日期的历史条目。
...
## 2026-04-13 18:05
- [x] 修复 `signal_dashboard_panel.py` 中 `self.last_update_label` 未定义导致的 `AttributeError` 崩溃：
  - [x] **补齐 UI 成员初始化**：在 `_init_ui` 的底部状态栏区域显式创建 `self.last_update_label`。
  - [x] **集成心跳时间显示**：将其接入 `status_layout` 布局，并配合 `_update_last_sync_time` 实现每秒级别的心跳更新显示，确保用户能直观确认数据同步的实时性状态，同时消除了结算后心跳检测触发的空对象引用风险。
