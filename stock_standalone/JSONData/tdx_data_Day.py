...
def calc_trend_channel(df, ur=6, lr=6):
    ...
    # ======== 模块 5 & 8: 委托通达信通道与 KX 上涨支撑线统一工厂 (SSOT) ========
    from JSONData.tdx_channel_factory import TDXChannelFactory
    ch_res = TDXChannelFactory.calculate(df, ur=ur, lr=lr)
    ...
    ch_pos = ch_res.ch_pos_series
    tc2 = ch_res.tc2
    bc2 = ch_res.bc2
    nod = ch_res.nod
    upper_price = ch_res.upper_price
    lower_price = ch_res.lower_price
    ...
    supp_slope_deg = ch_res.supp_slope_deg
    supp_pos = ch_res.supp_pos
    supp_days = ch_res.supp_days
...
def get_tdx_Exp_day_to_df(
    ...
    # 5. 核心指标（极速）
    # =========================
    with timed_ctx("get_tdx_macd", warn_ms=2000):
        df = get_tdx_macd(df, detect_calc_support=detect_calc_support)
    
    with timed_ctx("compute_lastdays_percent", warn_ms=2000):
        df = compute_lastdays_percent(
            df, lastdays=lastdays,
            resample=resample, normalized=normalized
        )
    ...
    if len(df) > 8:
        try:
            ...
            # df['high4'] = df.close.shift(2).rolling(4).max()
...
def get_sina_data_code(code,index=False):
    # index_status=False
    if not index:
        cname = sina_data.Sina(readonly=True).get_code_cname(code)
    else:
        cname = sina_data.Sina(readonly=True).get_stock_code_data(code,index=index).name[0]
    return cname
...
def getSinaAlldf(market='kcb', vol=ct.json_countVol, vtype=ct.json_countType, filename='mnbk', table='top_now', trend=False, readonly=True):
    ### Sina获取 Ratio 和tdx数据
    ...
    if isinstance(market, list):
        code_l = market 
        df = sina_data.Sina(readonly=readonly).get_stock_list_data(code_l)
        ...
        df = sina_data.Sina(readonly=readonly).get_stock_code_data(['999999', '399006', '399001'], index=True)
    ...
    #     df = sina_data.Sina().get_stock_list_data(code_l)
        ...
        # df = rl.get_sina_Market_json(market)
        df = sina_data.Sina(readonly=readonly).market(market)
        # df = sina_data.Sina().market(market)
    elif market in ['all','bj']:
        df = sina_data.Sina(readonly=readonly).all
        if market in ['bj']:
        ...
        df = sina_data.Sina(readonly=readonly).get_stock_list_data(codelist)
    else:
        if filename == 'cxg':
            df = wcd.get_wcbk_df(filter=market, market=filename,
        ...
        df = sina_data.Sina(readonly=readonly).get_stock_list_data(df.index.tolist())
    ...
    if len(m_mark) > 1:
        ...
        if not isinstance(m_0,list):
            ...
            dfw_codel = m_0
        dfw = sina_data.Sina(readonly=readonly).get_stock_list_data(dfw_codel)
    ...
    for da in range(1, lastdays + 1):
        ...
        for col, prefix in COL_MAPPING.items():
            ...
            if col in df.columns and len(df) >= idx:
                ...
                val = 0
            # upper 不加 d 后缀
            if prefix == 'upper' or prefix =='high4':
