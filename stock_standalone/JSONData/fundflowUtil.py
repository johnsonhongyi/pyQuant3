...
def get_dfcfw_fund_flow2020(market):
    ...
    log.info("url:%s"%(url))
    # data = cct.get_url_data_R(url,timeout=20).split('=')
    data = cct.get_url_data_requests(url,timeout=3,retry=1).split('=')
    # vollist=re.findall('{data:(\d+)',code)
    # vol_l = []
    log.debug(f"market data:{data}")
    ...
    for i in range(len(indexall)):
        ...
        if not single:
            url = ct.SINA_JSON_API_URL % ct.INDEX_LIST[indexall[i]]
            # data = cct.get_url_data_R(url,timeout=20,headers=sinaheader)
            data = cct.get_url_data_requests(url,timeout=3,headers=sinaheader,retry=1)
            vol_l = re.findall('\"([\d\D]+?)\"', data)
            if len(vol_l) == 1:
                data = vol_l[0].split(',')
                try:
        ...
        if len(vol_l) == 2:
            ...
            if len(df['amount']) > 0:
                ...
                if isinstance(dd['svol'], str) and dd['svol'].find('-') == 0:
                    ...
                    dd['allvol'] = "%s-%s-%s" % (round(dd['svol']+dd['zvol'],1), round(svol_v+zvol_v,1), round((svol_r+zvol_r)/2,1))
                    # dd['allvol'] = "%s-%s-%s" % (dd['svol']+dd['zvol'], round(svol_v+zvol_v,1), round((svol_r+zvol_r)/2,1))
...
def get_dfcfw_rzrq_SHSZ2(url=ct.DFCFW_RZYE2sh):
    ...
    df['all'] = list(map(lambda x, y: round(
        ...
        try:
            ...
            if len(row) >= 1:
                ...
                for tag in row[0].find_all('tr', attrs={"class": True}):
                    ...
                    if m_status_t in list(status_dict.keys()):
