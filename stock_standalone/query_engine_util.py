...
class PandasQueryEngine:
    ...
    def _prepare_context(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        col_map = {
            ...
            'ch_pos': ['ch_pos', 'channel_pos', 'ch_pct', '通道位置'],
            'ch_dir': ['ch_dir', 'channel_dir'],
            'ch_width': ['ch_width', 'channel_width'],
            'ch_height': ['ch_height', 'channel_height', 'ch_height_val', '通道高度'],
            'ch_height_pct': ['ch_height_pct', 'channel_height_pct', 'ch_amplitude_pct', 'ch_bandwidth_pct', '通道振幅', '通道高度振幅'],
            'ch_width_pct': ['ch_width_pct', 'channel_width_pct', 'ch_rel_width', '相对通道跨度', '通道相对跨度'],
            'upper_height': ['upper_height', 'ch_upper_height', '上半通道高度'],
            'lower_height': ['lower_height', 'ch_lower_height', '下半通道高度'],
            'bandwidth': ['bandwidth', 'boll_bandwidth', '布林带宽'],
            'bandwidth_pct': ['bandwidth_pct', 'boll_bandwidth_pct', 'relative_bandwidth', '相对带宽', '相对带宽比率'],
            'bollpect': ['bollpect', 'boll_pct', 'boll_pos'],
            'boll_sq': ['boll_sq', 'boll_strength_quant'],
            ...
            'upper_price': ['upper_price', 'ch_anchor_high_price', 'ch_high_price'],
            'ratio': ['ratio', 'turnover', 'turnover_rate', '换手', '换手率'],
            'turnover': ['ratio', 'turnover', 'turnover_rate', '换手', '换手率'],
            'vol_ratio': ['vol_ratio', 'volume_ratio', 'vr', '量比'],
            'cdp_support': ['cdp_support', 'cdp_supp', 'cdp支撑'],
            'cdp_reversal': ['cdp_reversal', 'cdp_rev', 'cdp反转'],
            'cdp_resistance': ['cdp_resistance', 'cdp_res', 'cdp阻力'],
            'cdp_breakthrough': ['cdp_breakthrough', 'cdp_break', 'cdp突破'],
        ...
        for i in range(0, 10):
            ...
            col_map[f'ch_upper{i}'] = [f'ch_upper{i}', f'ch_upper_{i}']
            col_map[f'ch_supp{i}'] = [f'ch_supp{i}', f'ch_supp_price{i}', f'ch_supp_{i}']
            col_map[f'ch_supp_price{i}'] = [f'ch_supp_price{i}', f'ch_supp{i}', f'ch_supp_price_{i}']
            col_map[f'ratio{i}'] = [f'ratio{i}', f'turnover{i}', f'换手率{i}', f'ratio_{i}', f'ratio{i}d', f'turnover{i}d']
            col_map[f'ratio{i}d'] = [f'ratio{i}d', f'ratio{i}', f'turnover{i}', f'换手率{i}']
            col_map[f'turnover{i}'] = [f'turnover{i}', f'ratio{i}', f'换手率{i}', f'turnover{i}d']
            col_map[f'vol_ratio{i}'] = [f'vol_ratio{i}', f'volume_ratio{i}', f'vr{i}', f'量比{i}', f'vol_ratio{i}d']
