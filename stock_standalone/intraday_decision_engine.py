...
class IntradayDecisionEngine:
    ...
    def evaluate(self, row: dict[str, Any], snapshot: dict[str, Any], mode: str = "full") -> dict[str, Any]:
        ...
        # 💥 [NEW] 提前评估早盘 60F 杀跌强反弹结构，用于后续风控与豁免
        ...
        # 💥 [FIX] 提前初始化周期阶段，防止 _realtime_priority_check 等早期调用使用旧数据
        self.cycle_stage = int(row.get('cycle_stage', snapshot.get('cycle_stage', 2)))
        debug["周期阶段"] = self.cycle_stage
        ...
        is_t1_restricted = buy_date.startswith(today_str) if buy_date else False
        debug["is_t1"] = is_t1_restricted
        ...
        # Priority Check 1 (High priority selling/buying conditions like open-down or VWAP breach)
        ...
        if mode in ("full", "buy_only"):
            # rebound_score and is_strong_rebound have been pre-calculated at the top of evaluate()
            ...
            # 【修复】如果均线决策返回“卖出”，且符合卖出模式，则执行
            if action == "卖出":
                if mode in ("full", "sell_only") and not is_t1_restricted:
                    debug["sell_reason"] = ma_reason
                    return {
                        "action": "卖出",
                        "position": 0.0,
                        "reason": ma_reason,
            ...
            if action == "买入":
                ...
                if structure == "派发":
                    # [优化] 如果是强反弹且价格在昨收上方，且日内回落未超过 5%，则豁免结构派发禁买
                    highest_today_val = float(snapshot.get("highest_today", price))
                    fall_from_high = (highest_today_val - price) / highest_today_val if highest_today_val > 0 else 0.0
                    is_exempt = is_strong_rebound and last_close > 0 and price > last_close and fall_from_high <= 0.05
                    if not is_exempt:
                        debug["refuse_buy"] = "结构为派发(冲高回落)"
                        return self._hold(f"结构{structure}禁买", debug)
                    else:
                        debug["结构派发豁免"] = f"回落{fall_from_high:.1%}在限额内"
                
                # (模式识别已移至上方)

                # 1. 应用基础过滤器
                base_pos += self._yesterday_anchor(price, snapshot, debug, is_strong_rebound)
                base_pos += self._structure_filter(row, debug, is_strong_rebound)
                base_pos += self._extreme_filter(row, debug, is_strong_rebound)
                ...
                if last_nclose > 0 and price < last_nclose:
                    ...
                    if support_score > 0.2:
                        ...
                        if not is_strong_rebound:
                            ...
                            # 强反弹且价格站在昨日收盘价上方，豁免该扣分
                            if last_close > 0 and price >= last_close:
                                debug["昨日锚点约束"] = "低于昨均价(强反弹豁免)"
                            else:
                                base_pos -= 0.10
                                debug["昨日锚点约束"] = "低于昨均价(强反弹折扣)"
                ...
                vwap_score = self._vwap_trend_check(row, snapshot, debug, is_strong_rebound)
                base_pos += vwap_score
                ...
                multiday_vwap_score = self._eval_multiday_intraday_structure(row, snapshot, debug, is_strong_rebound)
                ...
                # 💥 [NEW] 板块批量资金攻击方向评估 (Batch Sector Volume Attack)
                batch_attack_score = self._eval_batch_sector_attack(row, snapshot, debug)
                base_pos += batch_attack_score
    ...
    def evaluate_dynamic(self, base_eval: dict, row: dict[str, Any], snapshot: dict[str, Any]) -> dict[str, Any]:
            ...
            # 💥 [FIX] 同步更新周期阶段，防止复测时逻辑漂移
            self.cycle_stage = int(base_eval.get("debug", {}).get("周期阶段", row.get("cycle_stage", 2)))
    ...
    def _stop_check(self, row: dict[str, Any], snapshot: dict[str, Any], debug: dict[str, Any]) -> dict[str, Any]:
        ...
        # 💥 [NEW] 早盘冲高杀跌破分时结构极速止损 (优先级最高)
        morning_stop = self._morning_structure_breakdown_stop(row, snapshot, debug)
        if morning_stop["triggered"]:
            return morning_stop
    ...
    def _limit_price_filter(self, row: dict[str, Any], debug: dict[str, Any]) -> tuple[bool, str]:
        """
        价格限制过滤器：防止在涨停板或跌停板错误交易 (适配科创板/创业板20%、ST板5%、主板10%)
        """
        ...
        limit_up_threshold = limit_pct - 0.15
        limit_down_threshold = - (limit_pct - 0.15)
        
        # 涨停板判定
        if percent > limit_up_threshold:
            return True, "涨停板禁止追高"
        
        # 跌停板判定
        if percent < limit_down_threshold:
            return True, "跌停板禁止抄底"
    ...
    def _yesterday_anchor(self, price: float, snapshot: dict[str, Any], debug: dict[str, Any], is_strong_rebound: bool = False) -> float:
        """昨日锚点惩罚"""
        penalty = 0.0
        last_close = float(snapshot.get("last_close", 0))
        last_nclose = float(snapshot.get("nclose", 0))
        ...
        if last_nclose > 0 and price < last_nclose:
            if not is_strong_rebound:
                penalty -= 0.15
            elif last_close > 0 and price < last_close:
                # 强反弹但价格低于昨收，折扣惩罚
                penalty -= 0.10
            else:
                # 强反弹且站稳昨收，豁免昨均偏离惩罚
    ...
    def _structure_filter(self, row: dict[str, Any], debug: dict[str, Any], is_strong_rebound: bool = False) -> float:
        """结构过滤"""
        penalty = 0.0
        price = float(row.get("trade", 0))
        ma60 = float(row.get("ma60d", 0))
    ...
    def _extreme_filter(self, row: dict[str, Any], debug: dict[str, Any], is_strong_rebound: bool = False) -> float:
        """极端指标过滤"""
        penalty = 0.0
    ...
    def _vwap_trend_check(self, row: dict[str, Any], snapshot: dict[str, Any], debug: dict[str, Any], is_strong_rebound: bool = False) -> float:
        """
        VWAP (均价) 趋势过滤器
    ...
    def _eval_multiday_intraday_structure(self, row: dict[str, Any], snapshot: dict[str, Any], debug: dict[str, Any], is_strong_rebound: bool = False) -> float:
        """
        [多日分时交易结构引擎]
        结合今日/昨日/前日分时 VWAP (成交均价) 重心线及日内平台，对多日分时结构进行综合评估。
        """
        price = float(row.get("trade", 0))
        nclose = float(row.get("nclose", snapshot.get("nclose", price)))
        last_nclose = float(snapshot.get("last_nclose", snapshot.get("nclose", 0)))
        nclose2d = float(snapshot.get("nclose2d", 0))
        
        score = 0.0
        status_list = []
        
        if nclose > 0 and last_nclose > 0:
            # 1. 均价重心多日连续抬升 (Multi-day VWAP Uptrend)
            if nclose >= last_nclose * 0.998 and (nclose2d == 0 or last_nclose >= nclose2d * 0.995):
                if price >= nclose:
                     score += 0.15
                     status_list.append(f"多日VWAP重心上移(今{nclose:.2f}>昨{last_nclose:.2f})")
                else:
                     score += 0.05
            ...
            # 2. 均价重心连续多日下移 (阴跌/杀跌模型)
                ...
                if price < last_nclose:
                    last_close = float(snapshot.get("last_close", 0))
                    if is_strong_rebound and last_close > 0 and price >= last_close:
                        # 针对强反弹已站回昨收的形态，豁免重心连续下移的大额扣分，仅扣 -0.05
                        score -= 0.05
                        status_list.append(f"多日VWAP下移且低于昨均，但强反弹站回昨收豁免")
                    else:
                        score -= 0.35
            ...
            # 3. 单日均价重心明显崩塌或破位
                ...
                if is_strong_rebound and last_close > 0 and price >= last_close:
                    score -= 0.05
                    status_list.append(f"多日VWAP破位，但强反弹站回昨收豁免")
                else:
                    score -= 0.25
                    status_list.append(f"⚠️多日VWAP破位(今{nclose:.2f}<昨{last_nclose:.2f})")
        
        if last_nclose > 0:
            multi_bias = (price - last_nclose) / last_nclose
            debug["multi_vwap_bias"] = round(multi_bias, 4)
            
        debug["多日分时结构"] = " | ".join(status_list) if status_list else "平稳"
        return score

    def _intercept_60f_chasing(self, row: dict[str, Any], snapshot: dict[str, Any], debug: dict[str, Any]) -> tuple[bool, float, str]:
        """
        [60F 迟滞追涨硬拦截器]
        解决痛点：在 60F 连续大涨拉升完成、偏离分时均线且处于高位时追涨，次日易遭遇冲高杀跌破分时结构。
        """
        price = float(row.get("trade", 0))
        high = float(row.get("high", price))
        pct_now = float(row.get("percent", 0))
        ...
        if pct_now > 3.8 and (vwap_bias > 0.028 or multi_bias > 0.050):
            ...
            if now_time > dt.datetime.strptime("09:42", "%H:%M").time():
                debug["60F追涨拦截"] = f"偏离均价过远(日内偏离{vwap_bias:.1%},多日偏离{multi_bias:.1%})"
                return True, -0.35, "⚠️[拦截60F迟滞追高-偏离多日分时均线过远]"
                
        if fall_from_high > 0.022 and pct_now > 2.5 and vwap_bias > 0.015:
            debug["60F追涨拦截"] = f"60F顶端冲高回落({fall_from_high:.1%})"
            return True, -0.30, "⚠️[拦截60F迟滞追高-高位回落无量跟进]"
    ...
    def _eval_batch_sector_attack(self, row: dict[str, Any], snapshot: dict[str, Any], debug: dict[str, Any]) -> float:
        ...
        """
        batch_attack = snapshot.get("sector_batch_attack", {})
        if not batch_attack and isinstance(snapshot.get("sector_stats"), dict):
            stats = snapshot.get("sector_stats", {})
            avg_pct = stats.get("avg_pct", 0)
            if avg_pct > 2.5:
                batch_attack = {"is_attacking": True, "rally_ratio": 0.40, "reason": "板块均涨>2.5%"}
        
        score = 0.0
        if batch_attack:
            is_attacking = batch_attack.get("is_attacking", False)
            rally_ratio = batch_attack.get("rally_ratio", 0.0)
            if is_attacking or rally_ratio >= 0.35:
                score += 0.20
                debug["批量资金攻击"] = f"板块集群共振(攻击比{rally_ratio:.0%})"
            elif rally_ratio < 0.10 and float(row.get("percent", 0)) > 3.0:
                score -= 0.20
                debug["批量资金攻击"] = f"⚠️孤狼诱多(板块攻击比仅{rally_ratio:.0%})"
                
        return score

    def _morning_structure_breakdown_stop(self, row: dict[str, Any], snapshot: dict[str, Any], debug: dict[str, Any]) -> dict[str, Any]:
        """
        [早盘冲高杀跌破分时结构-极速止损避险]
        解决痛点：次日早盘冲高诱多后快速杀跌砸破日内/多日分时结构，避免传统硬止损滞后。
        """
        price = float(row.get("trade", 0))
        open_p = float(row.get("open", price))
        high = float(row.get("high", price))
        nclose = float(debug.get("nclose", snapshot.get("nclose", price)))
        last_nclose = float(snapshot.get("last_nclose", snapshot.get("nclose", 0)))
        
        now_time = dt.datetime.now().time()
        if dt.datetime.strptime("09:30", "%H:%M").time() <= now_time <= dt.datetime.strptime("10:20", "%H:%M").time():
            highest_today = float(snapshot.get("highest_today", high))
            if highest_today > open_p * 1.012 or highest_today > last_nclose * 1.015:
                drop_from_high = (highest_today - price) / highest_today
                if drop_from_high > 0.020:
                    if price < open_p and price < nclose and (last_nclose == 0 or price < last_nclose * 0.997):
                        return {
                            "triggered": True,
                            "action": "极速止损",
                            "position": 0.0,
                            "reason": f"⚡[早盘冲高杀跌破日内/多日分时结构-极速止损避险](高点回落{drop_from_high:.1%})"
                        }
        return {"triggered": False, "action": "", "position": 1.0, "reason": ""}
