...
class StockMonitorApp(DPIMixin, WindowMixin, TreeviewMixin, tk.Tk):
    ...
    def _inject_focus_engine(self, full_df):
        ...
        def _do_inject():
            try:
                ...
                # [DECOUPLE] 后台实时自动决策与买卖执行
                try:
                    self.bg_kernel_auto_execute_once(auto_mode=True)
                except Exception as _bg_exec:
                    logger.debug(f"[BgKernel] real-time auto execution failed: {_bg_exec}")
    ...
    def _bg_get_realtime_price_map(self, codes=None):
        ...
        import pandas as pd
        price_map = {}
        df_rt = getattr(self, 'df_all', None)
        ...
        try:
            ...
            logger.warning(f"Error in _bg_get_realtime_price_map: {e}")
        return price_map
    ...
    def bg_kernel_auto_execute_once(self, auto_mode=True):
        ...
        try:
            ...
            focus_ctrl = get_focus_controller()

            if not trade_gw or not focus_ctrl:
                return

            # 动态初始化今日去重缓存，挂载在持久单例 MonitorTK 实例上
            today_str = datetime.now().strftime("%Y-%m-%d")
            if not hasattr(self, "_bg_kernel_last_trade_date") or self._bg_kernel_last_trade_date != today_str:
            ...
            # 刷新模拟持仓的实时价格，并进行止损触发监测
            price_map = self._bg_get_realtime_price_map()
            if price_map:
            ...
            # 决定当前运行模式：如选股面板打开则同步其所选模式，否则默认 PAPER 模拟
            mode = "PAPER"
            win = getattr(self, "_stock_selection_win", None)
            if win and win.winfo_exists() and hasattr(win, "_kernel_mode_var"):
                mode_text = win._kernel_mode_var.get()
            else:
                mode_text = getattr(self, "_bg_kernel_mode", "PAPER 模拟")
            ...
            for sig in signals:
                ...
                price = float(sig.get('suggest_price') or sig.get('current_price') or targeted_price_map.get(code, 0) or 0)
                if price <= 0:
                    logger.warning(f"[BgKernel] 交易决策 {code}({sig.get('name', '')}) 缺少实时价格数据，已拦截。")
                ...
                if mode == "OBSERVE":
                    ...
                    focus_ctrl.decision_queue.update_status(code, "已观察")
                    records.append({
                        "code": code,
                        "name": sig.get('name', ''),
                        "action": action,
                ...
                # B. CONFIRM (弹窗确认模式)
                    ...
                    if code in self._bg_kernel_today_confirmed:
                        continue
                    if code in self._bg_kernel_today_ignored:
                        continue
                    ...
                    if win and win.winfo_exists():
                        self._schedule_after(0, lambda s=sig, a=action, p=price: win._show_kernel_confirm_dialog(s, a, p))
                        ...
                        logger.warning(f"[BgKernel] CONFIRM 模式决策 {code}({sig.get('name', '')}) {action} 拦截：选股面板未打开，无法进行确认。")
                    continue
                ...
                if action in ("BUY", "ADD"):
                    ...
                    if is_live:
                        if code in self._bg_kernel_today_buys:
                    ...
                    if is_live:
                        if code in self._bg_kernel_today_sells:
            ...
            if executed or errors:
                logger.info(f"⚡ [BgKernel] {msg}")

            if win and win.winfo_exists():
                self._schedule_after(0, lambda m=msg, k="ok" if executed else ("error" if errors else "warn"), rc=records: win._bg_sync_ui_from_kernel(m, k, rc, executed_codes, blocked_codes, error_codes))
    ...
    def _build_ui(self, ctrl_frame):
        ...
        self.search_history5 = self._format_history_list(h5, self.search_map5)
                            ...
                            try:
                                ...
                                if vis_enabled and not sent:
                                    with timed_ctx("viz_IPC_send", warn_ms=1000):
                                        try:
                                            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                                                s.settimeout(2.0)  # 增加到 2.0 秒，防止大包超时
                                                s.connect((ipc_host, ipc_port))
            ...
            if arg_key in kwargs:
                ...
                if source == "use" and selected_query and arg_key[-8:] == self.query_manager.current_key:
                    ...
                    if key_suffix in ["history3", "history4", "history5"]:
                        ...
                        self.search_var1.set(selected_query)
                        self.apply_search()
                    elif key_suffix == "history1":
                        # 回归正常的 history1，清空临时状态
                        self._temp_history_source = None
                        self._temp_history_query = ""
                        self.search_var1.set(selected_query)
                        self.apply_search()
                    elif key_suffix == "history2":
                        # 正常的 history2 条件
                        self.search_var2.set(selected_query)
                        self.apply_search()
                    else:
    ...
    def sync_history(self, val, search_history, combo, history_attr, current_key):
        ...
        if len(original_history) != len(new_history):
            ...
            for orig, new in zip(original_history, new_history):
                if orig.get("query") != new.get("query") or orig.get("note") != new.get("note") or orig.get("starred") != new.get("starred"):
                    is_changed = True
                    break

        setattr(self.query_manager, history_attr, new_history)
        if is_changed:
            self.query_manager._history_changed = True
    ...
    def apply_search(self):
        ...
        # 安全自愈机制：如果顶部的 query 已被手动修改，则清空临时状态
        ...
        if temp_src:
            ...
            if temp_src == "history3" and hasattr(self, 'search_map3'):
                ...
                current_q = self.search_map1.get(current_q, current_q)
                
            last_temp_q = getattr(self, "_temp_history_query", "")
            if current_q != last_temp_q:
                self._temp_history_source = None
                self._temp_history_query = ""
                temp_src = None
        ...
        if temp_src == "history3":
            ...
            if hasattr(self, 'search_map5'):
                val1 = self.search_map5.get(val1, val1)
        else:
