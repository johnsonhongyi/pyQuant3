...
a = Analysis(
    ...
    hiddenimports=[
        ...
        'JohnsonUtil.commonTips','talib.stream', 'talib.abstract', 'talib',
        'JSONData.tdx_data_Day', 'a_trade_calendar', 'tdx_utils'
    ],
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],
    excludes=[
        ...
        'bokeh', 'seaborn', 'flask', 'django', 'sqlalchemy', 'pyecharts', 'zmq', 'tornado'
    ],
    noarchive=False,
    optimize=1,
)
...
print(f"\n[MultiPeriodDialog Spec Optimizer] Filtered out {len(removed_binaries)} binary files.")
print(f"[MultiPeriodDialog Spec Optimizer] Filtered out {len(removed_datas)} data files.\n")
