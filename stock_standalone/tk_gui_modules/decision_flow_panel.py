        ...
        try:
            ...
            if service.paper_adapter is not None:
                try:
                    from trade_gateway import get_trade_gateway
                    trade_gw = getattr(self.parent_app, "_trade_gw", None) or get_trade_gateway()
                    if trade_gw and hasattr(service, "paper_adapter") and service.paper_adapter:
                        df_rt = getattr(self.parent_app, "df_all", None) if self.parent_app else None
                        service.sync_with_legacy_gateway(trade_gw=trade_gw, df_rt=df_rt)
