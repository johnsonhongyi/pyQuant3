...
def find_multi_period_leaders(multi_top_all: Dict[str, pd.DataFrame], query_rule: str) -> List[str]:
    """从大周期向小周期捕获热点龙头"""
    result_sets = []
    # 按照从大到小周期逻辑过滤
    for res in ['m', 'w', '3d', 'd']:
        if res in multi_top_all and not multi_top_all[res].empty:
            try:
                matched = multi_top_all[res].query(query_rule)
                if not matched.empty:
                    result_sets.append(set(matched.index))
            except Exception as e:
                logger.warning(f"Query failed for {res}: {e}")
    
    if not result_sets:
        return []
    # 取交集：满足所有大周期的强势条件
    final_codes = set.intersection(*result_sets)
    return list(final_codes)

def calc_multi_period_rank(multi_top_all: Dict[str, pd.DataFrame]) -> pd.Series:
    """计算综合强势分，权重：m=4, w=3, 3d=2, d=1"""
    weights = {'m': 4, 'w': 3, '3d': 2, 'd': 1}
    combined_score = None
    
    for res, w in weights.items():
        if res in multi_top_all and not multi_top_all[res].empty:
            df = multi_top_all[res]
            # 综合 TrendS (趋势得分) 和 win (连续上涨天数)
            score = (df.get('TrendS', 0).astype(float) * 0.7 + df.get('win', 0).astype(float) * 0.3) * w
            if combined_score is None:
                combined_score = score
            else:
                combined_score = combined_score.add(score, fill_value=0)
    
    if combined_score is not None:
        # 归一化到 0-100 方便显示
        max_score = combined_score.max()
        if max_score > 0:
            combined_score = (combined_score / max_score * 100).round(1)
...
def _fetch_market_data(market: str) -> pd.DataFrame:
    """获取实时行情副本"""
...
def _process_single_resample(
    ...
    if top_all.empty:
        if lastpTDX_DF.empty:
            with timed_ctx(f"init_lastp_{resample}", warn_ms=1000):
            ...
            with timed_ctx(f"update_lastp_{resample}", warn_ms=1000):
        ...
        with timed_ctx(f"combine_{resample}", warn_ms=1000):
            top_all = cct.combine_dataFrame(
                top_all, top_now, col="couts", compare="dff"
            )

    with timed_ctx(f"calc_{resample}", warn_ms=1000):
    ...
    with timed_ctx(f"filter_{resample}", warn_ms=800):
        df_out = (
    ...
    return top_all, lastpTDX_DF, df_out
...
# def fetch_and_process(shared_dict: Dict[str, Any], queue: Any, blkname: str = "boll", 
    ...
    while True:
        try:
            time_s = time.time()
            resample_active, market, st_key_sort, state = _prepare_runtime_state(
                logger, g_values, flag,
                resample, market, st_key_sort,
                marketInit, marketblk
            )
            ...
            # 只有交易时间且过了 init 阶段才运行
            if START_INIT > 0 and not cct.get_work_time():
                time.sleep(5)
                continue

            top_now = _fetch_market_data(market)
            if top_now.empty:
                time.sleep(2)
                continue
            ...
            for res in resamples:
                # 只有 active 的周期或者尚未初始化的周期才必须更新
                # 为了节省资源，非 active 周期可以每 N 次循环更新一次，但目前按用户要求保留全周期
                multi_top_all[res], multi_lastp_df[res], df_out = _process_single_resample(
                    top_now, res,
                    multi_lastp_df.get(res, pd.DataFrame()), 
                    multi_top_all.get(res, pd.DataFrame()),
                    detect_val, st_key_sort
                )
                multi_out[res] = df_out

            # 处理 Rank 和跨周期逻辑
            rank_series = calc_multi_period_rank(multi_top_all)
            
            # 组合输出包
            output_packet = {
                'active_resample': resample_active,
                'data': multi_out[resample_active],  # 当前显示的主周期（已过滤）
                'multi_periods': multi_top_all,      # 所有周期的完整数据（未过滤，用于跨周期查询）
                'rank': rank_series                  # 综合评分
            }
            
            logger.info(f"[数据包构建] active_resample={resample_active}")
            logger.info(f"[数据包构建] data shape={multi_out[resample_active].shape}")
            logger.info(f"[数据包构建] multi_periods keys={list(multi_top_all.keys())}")
            ...
            logger.info(f"Multi-period update done. Active:{resample_active} Packet keys:{list(output_packet.keys())}")
            time.sleep(1)
