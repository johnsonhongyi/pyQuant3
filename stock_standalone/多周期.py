...
可以,需要用好可视化的数据,day_df有全部基础数据ohlc和volume等,self.df_all有最近的cct.compute_lastdays




可以对信号的准确描述进行升级,这个📝 完整理由:
锌业股份 价格突破 6.19 涨幅 -10.03 量能 6.5 换手 0.6

🔍 信号细节 (迭代标尺):应该是跌破支撑位你知的大杀跌结构,今天查看了交易信号,遇到很严重的问题,都是只按今天的价格在做信号,而对前几日的涨跌走势,价格高低运行,以及放量滞涨,大阳后低开低走完全没有评估进去,导致各种进去买套的建议,今天成功了一只是300265,通光线缆,是开盘即最低,走高后突破昨日高点,前日高点,一直沿着均价线稳步持续拉升.在早盘听到语音播报没有窗口,在日志中查到买入的,这才是真正有价值的信号,后续的信号需要跟踪这只股的前置信号在处理才有信号的价值,整理优化这部分规则,统一信号及如何盘中实时跟进迭代.个股符合那种信号,,迭代,如低开走高,是否一直持续,买卖点



开始查看交易数据库及信号数据库,交易记录,最近交易非常的少,交易系统需要大升级,全部核心逻辑就是模拟早盘竞价买入后开盘几分钟是早盘最低点后收盘是盈利的单,当交易者每日在选中标的集合竞价买入后当天大部分时间都是盈利状态,尾盘也是确认盈利确认为有效单,连续开盘几分钟买入的价格是收益的为强势状态,当一个人面对开盘后未知的走势都是寄希望于观察走势决定买入,但是已经失去了先手的优势,只有明确的主力资金意图才会集合竞价买入,反之开盘几分钟后就是全天最高价,持续低于分时均价线运行就是主跌的卖单,需要竞价卖出,等观察到后已经晚了,所以交易系统升级为预设模式验证后对个股进行实践操作,主升浪的个股每日只有开盘几分钟是完美买点,把这个逻辑规划升级到交易体系中,放开交易,快速迭代优化,可以根据启动阳,进行竞价买入逻辑,可以根据反转K启动,当天开盘是低价拉升的都可以快速买入,反之卖出