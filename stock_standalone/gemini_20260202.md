...
| 日期 | 内容 | 影响 |
|------|------|------|
| 2026-05-30 15:15 | **绝对路径化统一与测试套件物理级加固**：全面 Hoist sys_utils 导入至顶部，移除全部局部导入；彻底清理 `kernel_service.py` 遗留的 `os.getcwd()`；在 `test_auto_ladder.py` 中为 `600519` 注入强势主升浪 mock 缓存指标，使测试全绿通过并解决 pytest 未知 mark 警告刷屏。 | `kernel_service.py`, `sys_utils.py`, `test_auto_ladder.py`, `test_paper_trading.py` |
