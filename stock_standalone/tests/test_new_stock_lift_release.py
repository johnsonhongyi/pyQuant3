...
class TestNewStockLiftRelease(unittest.TestCase):
    ...
    def test_01_fetch_restricted_release_calendar(self):
        ...
        for c in test_codes:
            if c in result:
                info = result[c]
                print(f"  标的 {c}: 解禁日={info.get('lift_date')}, 批次={info.get('lift_stage')}, 股份={info.get('lift_shares')}万股, 占比={info.get('lift_ratio')}%, 类型={info.get('lift_type')}")
                self.assertIn("lift_date", info)
                self.assertIn("lift_shares", info)
                self.assertIn("lift_ratio", info)
                self.assertIn("lift_type", info)
                self.assertIn("lift_batch_idx", info)
                self.assertIn("lift_stage", info)
    ...
    def test_02_combined_new_stocks_contains_lift_data(self):
        ...
        # 检查字段是否注入
        required_cols = ["lift_date", "lift_shares", "lift_ratio", "lift_type", "lift_batch_idx", "lift_stage"]
        ...
        if not valid_lift_df.empty:
            ...
            print(f"  样本标的 {sample['code']} {sample['name']}: 解禁日={sample['lift_date']}, 轮次={sample.get('lift_stage')}, 占比={sample['lift_ratio']}%")
    ...
    def test_03_ui_headers_and_preview_card(self):
        ...
        # 测试推演卡片包含解禁日信息与批次信息 (如 首次解禁 1/2批)
        mock_row_with_lift = {
            "code": "301689",
            "name": "富特科技",
            "issue_price": 14.00,
        ...
        print(f"  推演卡片标题 (带解禁批次): {card_title}")
        self.assertIn("最近解禁: 2027-09-04 (首次解禁 1/2批, 占比38.5%)", card_title)
