...
## 2026-07-27 14:28
- [x] **完成最新多周期策略与配置文件全量线上/生产环境同步落盘**：
    - [x] **全路径 MD5 校验 100% 一致**：将最新包含 `tpl_rebound_staircase_breakout`（异动回调整固起爆解套）和 `tpl_pre_breakout_staircase_layout`（起爆前1-2天低吸伏击）的 `multi_period_strategies.json` 策略文件，完整同步覆盖至 `stock_standalone/config`、`stock_standalone/dist/config`、`build/instock_MonitorTK.dist/config`、`D:\JohnsonProgram\instockMonitorTK\config` 及 `D:\JohnsonProgram\instockMonitorTK\0719\config` 全量线上/打包/发布路径。
    - [x] **校验结果**：全量 6 处配置文件副本的 MD5 (`a9087c06291207edb78b3ae7e9339044`) 与 24 套内置策略全量同步完成。
...
- [x] **拓展企稳策略至周线(w)/3D/2D大周期成交量极缩数据流 (`multi_period_strategies.json`)**：
    - [x] **大周期缩量数据流引入**：响应缩量数据流采样时间太短的痛点，在 `w` (周线) 周期中引入 `lastv1d <= 0.85 * lastv2d or lastv1d < lastv3d`；在 `3d` 周期中引入 `{or: lastv{1-2}d < 0.72 * lastv{3-5}d}` 多级大周期量能萎缩比对。
    - [x] **全维度地量覆盖**：实现从日线微观地量到周线/3日线大周期级量能见底的全维度数据流覆盖，大幅防范漏掉数周地量极缩洗盘的优质个股。
    - [x] **三方物理副本同步**：完成配置文件落盘并同步至 build 与 dist 目录。

## 2026-07-26 21:18
- [x] **重构 `tpl_trend_breakdown` 策略为大涨大跌后极度缩量+振幅收窄企稳低吸策略 (`multi_period_strategies.json`)**：
    - [x] **策略重构与定义转换**：将原有负面的“高风险破位”告警策略，重新打造为专门捕获**“大幅上涨后经历深度杀跌回调，并在底部强支撑区实现连续缩量地量、振幅剧烈收窄 (地平星线整固)”**的高胜率企稳潜伏买点策略。
    - [x] **多周期判定架构**：
        - **`m` 月线**：跌至 `MA10d` / `lower` 布林下轨支撑或保持 MACD 修复，确保位于大结构底部支撑区间；
        - **`3d` / `2d` 周期**：`abs(per) < 3.8%` 十字星波动，连续 2~3 个周期成交量极度萎缩，卖盘抛压彻底干涸；
        - **`d` 日线周期**：近 1~3 天振幅极度收窄 (`{and: abs(per{1-3}d) < 3.8}`)，收盘稳定在 `MA20d` / `MA60d` 均线之上，限制 `percent < 5.0%` 控制买入成本。
    - [x] **三方物理副本同步**：完成配置文件落盘并同步至 build 与 dist 目录。

## 2026-07-26 13:52
- [x] **重构并校准 `dff3` (大底累计涨幅) 与 `dff2` (距离 MA10 乖离贴合度) 正确指标逻辑 (`multi_period_strategies.json`)**：
    - [x] **校准真实定义与形态表达**：
        - **`dff3`**：代表大周期底部最低点（大底）上来的累计涨跌幅度。
        - **`dff2`**：代表经过震荡整固后，股价距离 MA10 10日均线的偏离距离（乖离贴合度）。
    - [x] **优化策略跟单形态**：
        - **防追高与均线跟单 (`dff2 < 15.0`)**：要求股价在震荡向上过程中，不能脱离 MA10 均线过远，确保具备安全的跟单买点。
        - **主升浪整固跟单 (`dff3 > 8.0 and dff2 < 12.0`)**：大周期已从底部走出一波向上趋势（`dff3 > 8.0`），但短线回踩/贴近 MA10 均线（`dff2 < 12.0`），提供极佳的主升低吸跟单契机。
    - [x] **完成策略配置落盘与物理副本同步**。

## 2026-07-26 13:50
- [x] **融合 `dff3` (大周期动量) 与 `dff2` (近两日动量) 动态对比算法至大结构龙头策略 (`multi_period_strategies.json`)**：
    - [x] **区分“刚起爆”与“主升中途回调”双形态**：
        - **刚刚起爆点 (`abs(dff3 - dff2) < 6.0` 或 `dff3 <= 1.3 * dff2`)**：大周期涨幅与近两日拉升高度贴合，说明大周期前期沉寂蓄势，近两日集中爆发放量起爆。
        - **主升中途回调跟单点 (`dff3 > dff2 + 4.0`)**：大周期已经形成主升趋势，近两日处于短线小幅微调/蓄势跟单点。
    - [x] **完成策略表达式集成与三方物理副本同步**：已成功融入 `tpl_multi_period_super_leader_momentum` 策略的日线筛选条件中。

## 2026-07-26 13:50
- [x] **新增多周期主升引领+日线强结构龙头策略 (`tpl_multi_period_super_leader_momentum`)**：
    - [x] **彻底解决旧策略“只抄底不跟强”问题**：基于立新能源 (001254)、艾艾精工 (603580)、锐捷网络 (301165)、恒尚节能 (603137) 等核心图例走势，打造专门用于捕获引领个股与强主升浪跟单的多周期共振策略。
    - [x] **多周期大结构主升共振**：月线/周线/3D 强制要求 `MA5 > MA10` 强斜率上升通道与布林上轨 UPPER 突破推升，拒绝弱势阴跌熊股。
    - [x] **日线超强结构+双模式跟单防护**：结合 `strong_structure_score > 68` 强势结构得分、`UPPER` 轨道突破与能量柱放大，同时涵盖“放量拉升突破”与“主升浪回踩 MA5 均线低吸跟单”，实现每日动态跟进、防漏掉任何全市场主升浪强势股。

## 2026-07-26 11:24
- [x] **解除反转起爆策略 2D 周期“水上MACD”硬枷锁，全面支持“大周期水下企稳未涨+日线小阳起爆”架构 (`multi_period_strategies.json`)**：
    - [x] **定位死锁根源**：定位发现此前 `tpl_bottom_oversold_wash_breakout` 在 `"2d"` 周期硬性规定了 `macddea > 0 and macddif > 0`（要求 MACD 必须水上多头），导致长城军工 (601606) 这种启动前大周期没大涨、在水下 (`macddif_2d < 0`, `macddea_2d < 0`) 底部企稳反转的经典结构在 `2d` 周期被误杀剔除，只有 `d` 周期能看到。
    - [x] **优化 2D 周期表达式**：将 `"2d"` 过滤重构为 `close >= 0 and (dif > dea or macd > macdlast1 or {or: macdlast{1-6} > 0})`。解除水上枷锁，全面兼容水下金叉、绿柱缩短及多级 MACD 修复，精准捕获“大周期筑底企稳未大涨、日线带量小阳起爆”第一买点。

## 2026-07-26 11:18
- [x] **优化 `tpl_bottom_oversold_wash_breakout` 反转起爆策略及全量预置策略日线量能比对管道 (`multi_period_strategies.json`, `verify_601606_strategy.py`)**：
    - [x] **深度对齐长城军工 (601606) 与北方长龙 (301357) 底层量能架构**：剖析发现北方长龙能匹配是因为前两天 `last2d` (200万) < `last3d` (247万) 且当天放量 1.39 倍；而长城军工启动前一天 `last1d` (3046万) 已爆量试盘洗盘，启动当天维持高位 3026万 涨停，导致旧表达式 `(lastv0d > 1.3 * lastv1d)` 与 `(lastv1d < lastv2d or lastv2d < lastv3d)` 狭隘单重对比被误杀。
    - [x] **重构放量与企稳洗盘多维判定**：在 `tpl_bottom_oversold_wash_breakout` 日线条件中引入多维量能对比管道 `(lastv0d > 1.25 * lastv1d or lastv0d > 1.35 * lastv2d or lastv0d > 1.35 * lastv3d or volume > 1.3)` 与地量企稳比对 `(lastv1d < lastv2d or lastv2d < lastv3d or lastv2d < 0.75 * lastv0d or lastv3d < 0.75 * lastv0d)`；同时修复了 `volume > 1.3 * lastv1d` 单位混用漏洞，全量覆盖“连续缩量后起爆”与“前一日试盘洗盘后次日高位强起爆”两种高胜率反转形态。
    - [x] **回归测试与物理评估 100% 通过**：经测试脚本验证，长城军工 (601606) 与北方长龙 (301357) 在新策略下均实现 100% 匹配与诊断通过。
...
- [x] **实现多周期策略 Hit 命中测试能力与编辑器分周期 Hit 命中数据同步高亮 (`multi_period_dialog.py`)**：
    ...
    - [x] **修复编辑器独立验证时的行情数据自动装载通道**：补充了 `sina_data.Sina(readonly=True).all` 的全量行情数据降级恢复机制。确保即便在编辑器独立打开或未运行全局筛选的场景下，点击“🔍 验证”也能秒级自动完成数据装载，100% 准确呈现对应周期的 `(🎯 Hit: N)` 命中数量。
...
- [x] **进一步优化分时放量强反弹买入决策与偏离约束豁免 (Further Optimized Intraday Strong Rebound Buy Decision & Deviation Penalties Exemption)**：
    ...
    - [x] **豁免/折扣昨日均价线约束惩罚**：在 `evaluate()` 昨日均价线约束中，针对 `is_strong_rebound` 的个股，若价格站稳在昨日收盘价上方，则全面豁免低于昨均价的 `-0.15` 惩罚；若价格仍在昨收下方，则折扣惩罚至 `-0.10`（而非 `-0.15`），降低偏离昨日重心对强反弹抄底买入的压制。
    - [x] **优化 `_yesterday_anchor` 偏离惩罚**：同步在 `_yesterday_anchor` 过滤器中传入 `is_strong_rebound`，当强反弹股价格站在昨日收盘价上方时，豁免低于昨均线带来的 `-0.15` 惩罚；在昨日收盘价下方时折扣至 `-0.10`。
    - [x] **折扣 `_vwap_trend_check` 反抽惩罚**：当 `_vwap_trend_check` 检测到均价重心下移且价格处于今日均价上方时（可能反抽），若个股符合 `is_strong_rebound` 强反弹特征，惩罚扣分由 `-0.10` 减半折扣至 `-0.05`。
    - [x] **优化 `_eval_multiday_intraday_structure` 连续下移惩罚**：针对重心多日连续下移或破位的弱势阴跌股，若日内触发 `is_strong_rebound` 强反弹且价格已站回昨日收盘价上方，则将大额结构惩罚（`-0.35` / `-0.25`）统一豁免并仅做 `-0.05` 的微弱趋势修正，彻底打通弱势超跌股在强反弹临界点的买入得分。
    - [x] **回测仿真闭环验证通过**：通过重跑 `688689 (银河微电)` 2026-07-21 仿真回测脚本，确认在早盘 `11:03:18` 正确触发了超跌放量强反弹买入信号（实时买入分 `0.50`，开仓仓位 `0.48`），且 14:30 后的尾盘建仓防范拦截器依旧能够精准执行拦截，回测信号完全符合预期。

## 2026-07-21 17:30
- [x] **优化分时强反弹在重度超跌趋势与结构指标中的评分惩罚豁免 (Optimized Trend & Structure Penalty Exemptions on Strong Rebounds)**：
    - [x] **实现评分惩罚精准豁免**：针对符合 `is_strong_rebound` 的个股，在 `evaluate()` 决策链中全面免除/豁免了 `_structure_filter` 带来的 `结构约束: -0.2`（突破结构惩罚）、`_extreme_filter` 带来的 `lower` 偏离度 `指标约束: -0.1`，以及 `trend_strength < -0.3` 时带来的 `trend_strength` 趋势走弱惩罚 `0.1`。
    - [x] **优化多日均价重心下移扣分**：在 `_eval_multiday_intraday_structure` 引擎中，针对强反弹已强势站回昨日均价的形态，豁免重心连续下移带来的 `-0.1` 扣分。
    - [x] **通过历史案例回测仿真闭环验证 (Backtest Closed-loop Verification)**：基于科创板个股 `688689 (银河微电)` 近期极端行情节点进行了三阶段仿真回测：
        - **节点一 (2026-07-03)**：尾盘高位大跌首日，成功在尾盘前拦截买入，给出【卖出/观望】决策。
        - **节点二 (2026-07-13)**：阴跌半山腰弱反弹/诱多，精准触发【卖出/观望】信号成功拦截。
        - **节点三 (2026-07-21)**：今日触底大长腿放量强反弹，成功触发【买入】决策，买入评分达到 `0.55`（开仓仓位 `0.48`），完美实现超跌强反弹抄底逻辑的自愈与优化。

## 2026-07-21 17:15
- [x] **优化分时强反弹豁免机制与防范尾盘建仓陷阱 (Optimized Intraday Rebound Exemptions & Anti-Tail-End Chasing Block)**：
    - [x] **实现分时放量强反弹的追涨拦截豁免**：在 `_intercept_60f_chasing` 中引入最低价跌幅与大长腿反弹比例判定。若日内最低点较昨收或日内高点跌幅超过 2.5%，且从低位反弹超过 50% 且放量，则判定为低位放量洗盘强反弹（即早盘急跌后的大长腿拉升），豁免 `vwap_bias` 和 `multi_bias` 的 60F 追涨硬拦截。
    - [x] **精细化强反弹的日内回落与开盘价限制**：当鉴定为强反弹结构且价格处于昨日收盘价上方时，最多宽限日内回落幅度至 5%（严格防范 8% 等重度冲高回落的顶部派发杀跌陷阱）；同时要求强反弹需站上今日均价线且高于昨收，方可豁免开盘 15 分钟后股价低于开盘价 of 硬性禁买限制，保证洗盘反弹的真企稳品质。
    - [x] **落地尾盘建仓防范拦截器 (`_intercept_tail_position_building`)**：针对 14:30 之后的买入信号实施严苛防范。若股价未涨停（距离涨停 > 0.5%），且处于均价线下方或缺乏板块主力共振（拉升无板块跟随的“孤狼”），或尾盘拉高偏离分时均价过远（`vwap_bias > 2.0%`），则予以强力扣分并硬性拦截买入，从源头上规避尾盘接盘、次日遭遇杀跌闷杀的风险。
    - [x] **在 TradingAnalyzer 中扩展尾盘拦截指标**：在 `trading_analyzer.py` 的指标展开中同步增加对 `tail_intercept` 标志位的解析，完成回溯优化闭环。

## 2026-07-21 16:10
- [x] **全面进化分时交易结构引擎与 60F 迟滞追涨/早盘急跌风控 (Evoluted Intraday Structure Engine & Anti-60F-Chase Risk Control)**：
    - [x] **升级多日分时 VWAP (成交均价) 交易结构引擎 (`_eval_multiday_intraday_structure`)**：结合今日/昨日/前日分时 VWAP (nclose/last_nclose/nclose2d) 连续重心比对，当检测到重心连续下移时，若价格未强势站回昨日均价，则判定为重心下移阴跌通道，予以硬熔断禁买；若属于强反弹结构且重新站上今日 VWAP，则予以豁免，实现阴跌中精准捕捉底部反转。
...
- [x] **重构多周期联动以使用 linkage_service 后台队列 (Optimized Linkage to use linkage_service queue)**：
    ...
    - [x] 在 Vis 可视化联动中同步支持了 favorites 的 `TIME_LINK` 时间轴状态匹配，彻底拉平了多周期与主终端的联动表现。
- [x] **板块个股列表键盘与双击联动支持 (Concept Stock List Key & Double-Click Linkage Support)**：
    - [x] 在 ConceptStocksDialog (板块个股列表对话框) 中将 self.table 的 stock_activated 信号安全连接至 self.parent().link_stock 方法（修复了原直接使用 parent 局部变量导致的 NameError 崩溃），全面支持了鼠标双击、键盘上下键移动以及选择变更时的全局行情联动 (Vis, Tdx, Ths 等)。
    - [x] 在 ConceptStocksDialog 填充完数据后自动聚焦表格并默认选中首个有效个股，大幅优化了无鼠标纯键盘操作下的复盘效率。
- [x] **DNA 专项审计报告列表键盘上下键联动支持 (DNA Audit Report Keyboard Linkage Support)**：
    - [x] 将 QtDnaAuditReportWindow (DNA 专项审计报告窗口) 的表格控件从 QTableWidget 升级为 BaseATSTableWidget。
    - [x] 将 self.table 的 stock_activated 信号连接至主窗口的 link_stock 方法。当用户在审计报告表格中按键盘上下键移动或双击行时，将即时激活后台异步 linkage 机制发送至外部终端，与主界面联动机制完美对齐。
    - [x] 在 _on_selection_changed 中剥离并移除了已失效的 on_code_click 遗留方法调用。
    - [x] 报告填充完毕后自动默认聚焦于表格并设置当前活动单元格为首行首列，确保键盘方向键直接接管导航。
- [x] **个股/审计报告列表涨跌幅标准配色优化 (Vibrant A-Share Red/Green Coloring for Dialog Stock Lists)**：
    - [x] 针对 ConceptStocksDialog (板块个股列表) 的 percent (涨幅%) 字段以及 QtDnaAuditReportWindow (DNA 报告列表) 的 波段涨幅% (total_pct) 字段，根据数值的正负情况，强制使用符合中国 A 股习惯的、清亮专业的 A 股红 (#ff4444) 和 A 股绿 (#33cc5a) 进行前景色渲染，废除了不好看的默认配色和无色状态。
    - [x] 同步优化了主窗口中星标自选股 (display_name) 和涨跌幅的配色展现。
- [x] **完全避免运行期写盘、将持久化推迟至窗口关闭 (Deferred Configurations Persistence to Window Close Only)**：
    - [x] 重新优化了 _save_state 方法：在常规交互（如策略切换、周期复选、自选联动等）时，仅在内存中更新 self.ui_state，完全屏蔽实时的文件写盘操作。
    - [x] 只有在窗口关闭 (closeEvent) 或程序显式退出时，才通过传入 FORCE_WRITE 参数真正触发 HDF5/JSON 文件的物理写盘。彻底消除了运行期由于频繁操作界面选项引发的磁盘锁竞争与卡顿。

## 2026-07-18 21:35

- [x] **极限性能优化：根治联动卡顿、双信号冗余与高频写盘 (Extreme Performance Optimization: Fixed Linkage Lag, Double-Signal & IO Spam)**：

    - [x] **根治 BaseATSTableWidget 双信号触发卡顿 (Eliminated Dual-Signal Double-Fire)**：废除了同时绑定 `itemClicked` + `itemSelectionChanged` 导致每次点击触发两次联动的根本缺陷，统一改为单一的 `currentCellChanged` 信号入口。此信号天然覆盖鼠标点击与键盘上下键导航，从源头消除了双重处理开销。

    - [x] **引入 BaseATSTableWidget 20ms 内置防抖器 (Built-in 20ms Debounce in BaseATSTableWidget)**：将防抖由 80ms 压缩至 20ms，在完全过滤键盘极速连按导致的数据风暴的同时，使单次鼠标点击与按键操作达到几乎“零延迟”的即时反应体验。

    - [x] **实现 IPC 联动后台守护线程异步化 (Asynchronous IPC Linkage Execution)**：重构了 `link_stock` 触发逻辑，将原先同步阻塞主 GUI 线程的 socket 连结 (Vis) 以及 `StockSender()._do_send` (Tdx/Ths) 处理流程整体移入后台 `daemon` 线程异步执行。结合 `pyqtSignal` 机制实现了线程安全的状态栏文字反馈，从根本上杜绝了任何操作和点击时“慢半拍”的黏滞感。

    - [x] **引入列头索引缓存消除O(N)重复扫描 (Cached Column Index Lookup)**：在 `BaseATSTableWidget` 中引入 `_code_col_cached`、`_name_col_cached`、`_col_cache_valid` 缓存机制，重写 `setColumnCount` 和 `setHorizontalHeaderLabels` 以在列结构变更时自动失效。使列头查找从每次点击O(N)扫描降低为O(1)字典查找。

    - [x] **增加同代码去重过滤器 (Same-Code Emission Guard)**：在 `_fire_linkage_debounced` 中维护 `_last_emitted_code` 状态，确保连续导航到同一行时不会重复发出 `stock_activated` 信号。

    - [x] **清除多余的 `currentCellChanged` 重复注册 (Removed Duplicate Signal Connection)**：删除了 `multi_period_dialog.py` 中额外注册到 `self.table.currentCellChanged` 的冗余槽函数 `_on_table_current_cell_changed`，以及重复的 `_do_link_stock_debounced` 方法，彻底消除双路联动路径。

    - [x] **强制所有写盘只在窗口关闭时执行一次 (Disk Write Only on Close)**：

        - **`resizeEvent`**：完全移除 `save_window_position_qt_visual` 调用，不再每次拖拽窗口大小时触发磁盘IO。

        - **`moveEvent`**：完全移除 `save_window_position_qt_visual` 调用，不再每次移动窗口时触发磁盘IO。

        - **`_on_section_resized`**：移除 `QTimer.singleShot(50, self._adjust_column_widths)` 级联写盘调用，改为只更新内存缓存 `_base_widths`。

        - **`closeEvent`**：统一所有写盘逻辑：`save_window_position_qt` + `_save_state` 各执行一次，并停止 `favorites_timer`，清理 worker 信号连接，级联关闭龙头监控窗口。

    - [x] **降低 favorites 轮询频率 (Reduced Favorites Poll to 3s)**：将自选股版本轮询从 500ms 延长至 3000ms，大幅减少每秒3-4次的 `GlobalFavoriteManager` 对象构造与版本比对开销。

    - [x] **删除多余 closeEvent 残留 (Removed Duplicate closeEvent)**：清理了之前错误注入导致的第二个 `closeEvent` 定义（位于 `_poll_favorites_loop` 之后），防止 Python 类方法覆盖导致真实的关闭逻辑失效。��移除 `QTimer.singleShot(50, self._adjust_column_widths)` 级联写盘调用，改为只更新内存缓存 `_base_widths`。

        - **`closeEvent`**：统一所有写盘逻辑：`save_window_position_qt` + `_save_state` 各执行一次，并停止 `favorites_timer`，清理 worker 信号连接，级联关闭龙头监控窗口。

    - [x] **简化 `link_stock` 为直接执行 (Simplified link_stock to Direct Execution)**：废除了 `link_stock` 内的二次 150ms `QTimer` 防抖（已由 base_table 的 80ms 防抖接管），改为直接执行 IPC 发送。保留同代码过滤，并在发送后重置 `table._last_emitted_code`，确保切换窗口再切回时同一股票仍可重新联动。Vis 失败静默处理，不再阻塞状态栏更新。

    - [x] **降低 favorites 轮询频率 (Reduced Favorites Poll to 3s)**：将自选股版本轮询从 500ms 延长至 3000ms，大幅减少每秒3-4次的 `GlobalFavoriteManager` 对象构造与版本比对开销。

    - [x] **删除多余 closeEvent 残留 (Removed Duplicate closeEvent)**：清理了之前错误注入导致的第二个 `closeEvent` 定义（位于 `_poll_favorites_loop` 之后），防止 Python 类方法覆盖导致真实的关闭逻辑失效。
