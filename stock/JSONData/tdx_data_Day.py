...
def compute_perd_df(dd, lastdays=3, resample='d'):
    ...
    last_TopR_days = ct.compute_lastdays if resample == 'd' else ct.compute_lastdays
...
def compute_cum_and_top_stats(df, ma_days_list=['5'], lastdays=ct.compute_lastdays, per_top_limits=[(9.9, 'top10'), (5, 'top5')]):
    """
    统一计算 MA 累计值和涨幅统计指标
...
def compute_ma5d_count_slow(df,lastdays=ct.compute_lastdays,madays='5'):
    # temp=df[df.columns[(df.columns >= 'ma%s1d'%(madays)) & (df.columns <= 'ma%s%sd'%(madays,lastdays))]][-1:]
...
def compute_top10_count_slow(df,lastdays=ct.compute_lastdays,top_limit=ct.per_redline):
