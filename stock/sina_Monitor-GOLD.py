        ...
        try:
            ...
            if len(top_now) > 10 and len(top_now.columns) > 4:
                ...
                if 'trade' in top_now.columns:
                    top_now['buy'] = (
                        list(map(lambda x, y: y if int(x) == 0 else x, top_now['buy'].values, top_now['trade'].values)))
                ...
                if len(codelist) > 0:
                    ...
                    top_all['dff'] = (
                        list(map(lambda x, y: round(((float(x) - float(y)) / float(y) * 100), 1), top_all['buy'].values, top_all['lastp'].values)))
                    ...
                    if cct.get_now_time_int() > 915 and cct.get_now_time_int() <= 926:
                        top_all['percent'] = (list(map(lambda x, y: round((x - y) / y * 100, 1)
