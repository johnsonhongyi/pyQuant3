...
# def open_alert_center(parent):
#     """报警中心窗口"""
#     manager = parent.alert_manager
...
#     # 规则表格
#     columns = ("stock", "cond_type", "threshold", "created_at")
#     tree = ttk.Treeview(win, columns=columns, show="headings")
#     tree.pack(fill="both", expand=True)

#     # 表头
#     tree.heading("stock", text="股票")
#     tree.heading("cond_type", text="条件")
#     tree.heading("threshold", text="阈值")
...
#         }
#         # 传给编辑器
#         from .alert_editor import open_alert_editor  # 根据你放的位置修改
#         open_alert_editor(parent, stock_info=stock_info, rule=rule, new_rule=False)
#         refresh_rules()

#     def new_rule():
#         from .alert_editor import open_alert_editor  # 根据你放的位置修改
#         open_alert_editor(parent, stock_info=None, new_rule=True)
#         refresh_rules()

#     menu.add_command(label="新建规则", command=new_rule)
#     menu.add_command(label="编辑规则", command=edit_selected)
#     menu.add_command(label="删除规则", command=delete_selected)
...
#     tk.Button(btn_frame, text="新建规则", command=new_rule).pack(side="left", padx=5)
#     tk.Button(btn_frame, text="删除规则", command=delete_selected).pack(side="left", padx=5)
#     tk.Button(btn_frame, text="关闭", command=win.destroy).pack(side="right", padx=5)
# --------------------------
# AlertManager 类
# --------------------------
