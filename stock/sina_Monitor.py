...
#                    by=ct.Monitor_sort_count, ascending=[0, 0, 0, 0, 1])
                ...
                if st_key_sort.split()[0] in ['1','7']:
                    ...
                    if len(_top_all) > 0 and _top_all.lastp1d[0] == _top_all.close[0] and _top_all.lastp1d[-1] == _top_all.close[-1]:
                        ...
                        #     # top_temp = top_all.query('lasth1d > upper1 and lasth2d > upper2*0.998 and open > lastp2d*0.998 and low >= open*0.998')
                        #     top_temp =  top_all.query('lasth2d > upper2 and (lasth3d > upper3*0.998 or (high > upper2 and lasth2d > lasth3d)) and open > lastp2d*0.998 and low >= open*0.998')
                            
                    else:
                        print('initf ',end='')
                        if cct.GlobalValues().getkey('initfilter') is None:
                            initfilter = cct.read_ini(inifile='filter.ini',category='sina_Monitor',filterkey='initfilter')
                        else:
            ...
            #         lastpTDX_DF = pd.DataFrame()
                ...
                if len(rule.split()) > 1:
                    ...
                    if rule.find('top_all') > 0:
                        ...
                        # print(f"defaultRule: top_all.query{cct.read_ini(inifile='filter.ini',setrule='default',category='sina_Monitor',filterkey=filterkey)}")
                ...
                if rule.lower().startswith('w') and  query_rule is not None and set_query is not None:
                    cct.read_ini(inifile='filter.ini',setrule=f'top_all.query{set_query}',category='sina_Monitor',filterkey=filterkey)
                else:
