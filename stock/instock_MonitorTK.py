...
def fetch_and_process(queue, blkname="boll"):
    global st_key_sort
    ...
    LoggerFactory.log.error(f"resample: {resample}")
    while True:
        try:
...
class StockMonitorApp(tk.Tk):
    ...
    def load_data_from_csv(self):
        ...
        if file_path:
            try:
                ...
                LoggerFactory.log.error(f"加载 CSV 失败: {e}")
    ...
    def save_window_position(self):
        ...
        try:
            ...
            LoggerFactory.log.error(f"保存窗口位置失败: {e}")
    ...
    def load_window_position(self):
        if os.path.exists(WINDOW_CONFIG_FILE):
            try:
                ...
                LoggerFactory.log.error(f"读取窗口位置失败: {e}")
