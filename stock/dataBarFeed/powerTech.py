...
def compute_perd_df(dd,lastdays=3):
    ...
    df['perlastp'] = list(map(cct.func_compute_percd2, df['close'], df['close'].shift(1), df['open'], df['open'].shift(1), df['high'].shift(1), df['low'].shift(1), df['high'], df['low'],df['volume'],df['volume'].shift(1),df['upper']))
...
def get_ene_status(code, df=None, start=None, end=None, dl=None, dtype='d', ptype='close'):
    ...
    today = dd[-1:]
    if  stsc > 0:
        print("stsc1upperT:", stsc,dd.date[-1:], end=' ')
