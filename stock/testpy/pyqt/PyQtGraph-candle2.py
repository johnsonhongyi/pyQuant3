...
class KLineWidget(KeyWraper):
    ...
    def showSig(self, datas, main=True, clear=False):
        ...
        if clear:
            ...
            if datas and not main:
                sigDatas = np.array(list(datas.values())[0])
    ...
    def loadData(self, datas, sigs=None):
        ...
        datas['time_int'] = np.array(list(range(len(datas.index))))
        self.datas = datas[
        ...
        datas0['time_int'] = np.array(list(range(len(datas.index))))
...
class Crosshair(QtCore.QObject):
    ...
    def plotInfo(self, xAxis, yAxis):
        ...
        for sig in self.master.sigData:
            ...
            html += '<span style="color: %s;  font-size: 12px;">&nbsp;&nbsp;%s：%.2f</span>' % (col, sig, val)
        html += '</div>'
