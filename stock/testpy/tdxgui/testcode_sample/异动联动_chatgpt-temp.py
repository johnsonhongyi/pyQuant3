...
def send_to_tdx(stock_code):
    ...
    print(tdx_state,ths_state,dfcf_state)
...
def open_alert_center():
    ...
    stock_code, stock_info = None, None
    selected_item = tree.selection()
    if selected_item:
    ...
    if stock_code not in monitor_windows.keys():
        vlist = list(monitor_windows.keys())
        vlist.append(stock_code)
        # top_frame.insert(0, stock_code)
        stock_entry = ttk.Combobox(top_frame, textvariable=stock_var, values=vlist, width=10)
    else:
...
def open_alert_editor(stock_code, new=False):
    ...
    # ------------------ 数据处理 ------------------
    # 简化数据获取，使其能正常运行
    price, percent, vol = 0.0, 0.0, 1
    if not stock_code == '':
        try:
            if isinstance(stock_code, (list, tuple)) and len(stock_code) >= 7:
                code, name, idx, typ, percent, price, vol = stock_code
            else:
                if stock_code in monitor_windows.keys():
                    stock_info = monitor_windows.get(stock_code, {}).get('stock_info', [stock_code, 0, 0, 0, 0, 0, 0])
                    code, percent, price, vol = stock_info[0], stock_info[4], stock_info[5], stock_info[6]
                else:
    ...
    if not rules or new:
        rules = [
            {"field": "价格", "op": ">=", "value": price, "enabled": True},
            {"field": "涨幅", "op": ">=", "value": percent, "enabled": True},
            {"field": "量", "op": ">=", "value": vol, "enabled": True},
        ]
    ...
    def add_rule(field="价格", op=">=", value=0.0, enabled=True):
        row = len(entries)
        
        # 启用/禁用 选择框
        enabled_var = tk.BooleanVar(value=enabled)
        enabled_check = ttk.Checkbutton(rules_frame, variable=enabled_var)
        enabled_check.grid(row=row, column=0, padx=(0, 5), sticky="w")
        
        # 字段选择
        field_var = tk.StringVar(value=field)
        field_combo = ttk.Combobox(rules_frame, textvariable=field_var, 
                                   values=["价格", "涨幅", "量", "市盈率", "换手率"], width=10)
        field_combo.grid(row=row, column=1, padx=5, pady=2, sticky="w")
        
        # 操作符选择
        op_var = tk.StringVar(value=op)
        op_combo = ttk.Combobox(rules_frame, textvariable=op_var, values=[">=", "<="], width=5)
        op_combo.grid(row=row, column=2, padx=5, pady=2, sticky="w")
        
        # 值输入
        val_var = tk.DoubleVar(value=value)
        spin = ttk.Spinbox(rules_frame, textvariable=val_var, from_=-100, to=100000, increment=0.01, width=15)
        spin.grid(row=row, column=3, padx=5, pady=2, sticky="ew")

        # 百分比调整按钮
        btn_minus = ttk.Button(rules_frame, text="-1%", command=make_adjust_fn(val_var, -0.01), width=4)
        btn_minus.grid(row=row, column=4, padx=2, pady=2)
        btn_plus = ttk.Button(rules_frame, text="+1%", command=make_adjust_fn(val_var, 0.01), width=4)
        btn_plus.grid(row=row, column=5, padx=2, pady=2)
        ...
        # 调整列权重，使值输入框可以扩展
        rules_frame.grid_columnconfigure(3, weight=1)

    for rule in rules:
        add_rule(rule["field"], rule["op"], rule["value"], rule.get("enabled", True))
...
def refresh_alert_center():
    ...
    for alert in alerts_history:
        alert_tree.insert("", "end", values=(alert['time'], alert['stock_code'], alert['rule'].get('field'),alert['rule'].get('op'),alert['rule'].get('value')))
        # alert_tree.insert("", "end", values=(alert['time'], alert['stock_code'], alert['name'], alert['msg'], str(alert['rule'])))
...
def check_alert(stock_code, price, change, volume):
    global alerts_rules,alerts_history
