...
def create_monitor_window(stock_info):
    ...
    """
        参数:
        window (tk.Tk/tk.Toplevel): 需要添加标题栏的窗口。
        title_text (str): 标题栏上显示的文本。
        height (int): 标题栏的高度 (单位: 像素)。
    """
    # # 移除原生标题栏
    # monitor_win.overrideredirect(True)
    # height = 15
    # title_text = f"监控: {stock_name} ({stock_code})"
    # # 创建自定义标题栏 Frame
    # title_bar = tk.Frame(monitor_win, bg='lightgray', height=height, relief='raised', bd=0)
    # title_bar.pack(fill='x', side='top')
    
    # # # 绑定拖动事件
    # # title_bar.bind("<B1-Motion>", move_window)
    
    # # 创建标题 Label，并根据需要调整字体大小来影响高度
    ...
    # # # 创建关闭按钮
    # close_button = tk.Button(title_bar, text='X', command=lambda: on_close_monitor(window_info), bg='red', fg='white', relief='flat')
    # close_button.pack(side='right')

    # # 创建主内容区
    # content_frame = tk.Frame(monitor_win, bg='white')
    ...
    # monitor_tree.column("涨幅", width=80, anchor=tk.W, minwidth=80)
    # monitor_tree.column("价格", width=80, anchor=tk.W, minwidth=80)
    # monitor_tree.column("量", width=80, anchor=tk.W, minwidth=80)
    ...
    # update_position_window(monitor_win,window_info['stock_info'])
