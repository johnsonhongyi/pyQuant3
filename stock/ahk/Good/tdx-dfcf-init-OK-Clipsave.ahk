...
CheckClipboard()
{
    ...
    ; 获取当前剪贴板文本
    current := Clipboard
    ;MsgBox, 0x40,, 剪贴板中检测到6位数字: %current%
    ; 如果当前文本和之前保存的不一样，说明有新内容
    if (current != ClipSaved && current != "") {
        ClipSaved := current  ; 更新全局变量
        if RegExMatch(ClipSaved, "^(?:60|30|00|43|83|87|92)\d{4}(?!\d)|^(?:688|200)\d{3}(?!\d)", found_match)

        {
            ...
            ;Notify2(剪贴板中检测到6位数字, "tooltip", 3)
            ;Notify1(剪贴板中检测到6位数字, "tooltip", 3)
            ;Notify2(剪贴板中检测到6位数字, "tray", 3)
            Notify1(剪贴板中检测到6位数字, "sound", 0.3)
            ; 你可以在这里添加其他操作，例如：
    ...
    return false  ; 没有变化或为空

    /*
        ; 在这里处理新剪贴板文本
        ToolTip, New clipboard content:`n%ClipSaved%
...
HandleClipboardChange(Type) {
    ...
    /*
    if (Type = 1) {  ; 文本类型变化
        ClipSaved := Clipboard  ; 更新全局文本内容
        ToolTip, Clipboard updated:`n%ClipSaved%
        SetTimer, RemoveTooltip, -1500
    }
    */
    ; 检查标志
...
#If WinActive("ahk_class TdxW_MainFrame_Class") || WinActive("ahk_class TdxW_SecondFrame_Class")
{
    ...
    #If WinActive("ahk_class TdxW_MainFrame_Class")
    {
        ...
        if (ErrorLevel) {
            ...
            ClipSaved := Clipboard
            ;Log("FetchCodeFromTDX: got clipboard: " . SubStr(ClipSaved,1,200))
        }
