...
# # 设置窗口标题
# tdx_ths_position={'通达信':'659, 50, 878,793','东方财富':'268, 0, 1067, 833','同花顺':'0, 20,945,864'}
tdx_ths_position={'通达信':'659, 50, 878,793','东方财富':'268, 0, 1067, 833','同花顺':'0, 20,1075,864','Firefox': '626, 0, 878, 869'}

def set_windows_positon(title):
    # window_title = "通达信金融终端V7.642 - [分析图表-平安银行]"
    window_title = title
    pos='659, 50, 878,793'

    for idx in tdx_ths_position.keys():
        if title.find(idx) >= 0:
            print(idx)
            pos = tdx_ths_position[idx]
    # print(pos)
    ...
    _pos = pos.split(',')
    x,y = int(_pos[0]),int(_pos[1])
    width,height = int(_pos[2]),int(_pos[3])
...
if __name__ == '__main__':
    ...
    title = 'Firefox'
    # title = '东方财富'
    proc_title = ['Firefox','通达信','东方财富','同花顺']

    # FindWindowRectFromName(title)

    # print(*list_user_windows(), sep='\n')
    win_info = list_find_windows(title)
    print(win_info[0])
    # set_windows_positon(win_info[0].title)
